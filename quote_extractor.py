...
NEWS VALUE: When ranking quotes for inclusion, prefer quotes that contain news (an opinion on a future move, a roster need, a player evaluation that affects trade/free-agency speculation, a specific coaching critique, an admission of regret, a specific prediction) over quotes that are pure analysis or recap. Both are extractable when they're editorially compelling, but news-value quotes are the ones HoopsHype readers seek out — they become trade rumor articles, headlines, social posts. A coach saying "we need another ball-handler" is news. The same coach saying "we played a system that worked" is analysis. Both can be included, but if you're choosing between two quotes of similar quality, take the news one.
